	...
	//fprintf(fp,"read %d events:\n", n);
        for (int i=0 ; i<n ; i++) {
			...
			if(data.type == SENSOR_TYPE_ACCELEROMETER) {
				sprintf(buf,"time=%lld, type=%d, sensor=%s, value=<%5.1f,%5.1f,%5.1f>\n",
				data.timestamp,
				acctype,                            	
			...
			else if(data.type == SENSOR_TYPE_GYROSCOPE)
			{
				sprintf(buf,"time=%lld, type=%d, sensor=%s, value=<%5.1f,%5.1f,%5.1f>\n",
                            	data.timestamp,
				gyrtype,
			...
			else if(data.type == SENSOR_TYPE_PRESSURE)
			{
				sprintf(buf,"time=%lld, type=%d, sensor=%s, value=%f\n",
				data.timestamp,				
				bartype,
                            	getSensorName(data.type),
